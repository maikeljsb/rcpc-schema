...
*Module `product` of `CAPABILITY-MAP.md`. Drafted, reviewed, and approved 2026-09-15 (`732ddaf`); the required-everywhere redesign in decision 2 and decision 15 followed during Task 2's implementation; `Material` and `made_of` added 2026-09-21 (decision 17); `ComponentPermanence` and `permanence` moved to common 2026-09-22 (decision 18). Depends on `common`. Imported by `process`. Direction fixed by the brief's Product Side and its dated decisions; IFC lineage from `docs/research/ifc43-for-product.md`; derivation limits from `docs/research/topologicpy-for-product.md`; metamodel usage follows `docs/research/linkml-metamodel-conformance-and-inheritance.md`.*

## Objective

Give the process side something to build: a building component identified by an IFC GlobalId, parsed or minted, made of a Material whose category a method matches on, the point a placement task binds to, and the light spatial topology that says where it is and what a robot passes through to reach it. Deliver it as `schema/product.yaml`, with example documents taken from a real model, generated JSON Schema, and generated documentation.

**Users.** The IFC parser and the derivation, later, which fill BuildingComponents, Spaces, Storeys, and Connectors from a model. The person resolving a parsed model, who adds derived components such as bricks and fills missing materials and categories. The mapping step, outside this module, that fills each `Material`'s `category`. The process module, whose methods match a Material's `category` and whose primitives reference components by id. The projection component, which turns `derived_from`, `part_of`, `located_in`, `contained_in`, and `connects` into edges. Other project teams, who read `docs/model/product/`.

**Success in one sentence.** `schema/product.yaml` lints clean, the five example documents from the real model validate and the eleven invalid documents fail naming their slot, the toolchain produces its JSON Schema and docs unchanged, and every row of the lineage table below names the IFC construct it borrows or adapts.
...
uv run linkml-validate -s schema/product.yaml -C Material examples/product/materials.yaml
...
examples/
  product/
    ...
    materials.yaml                                      one record per distinct material string, with its category
    invalid/
      ...
      material_missing_category.yaml                    a Material without the category key
dist/
...
tests/
  test_examples.py                                      sixteen rows added to EXAMPLES, nothing else
```
...
`MaterialCategory` in common, a class with its document `material_categories.yaml` (`SPEC-common.md` decisions 15 and 17), added 2026-09-21 in its own commit before `Material` lands here; `MaterialName`, which `material` ranged, is retired in process's commit, once nothing ranges it. `Position` with its unit and the four dimension slots already existed.
...
Every record's `id` is an IFC GlobalId, the 22-character compressed form, and the pattern `^[0-3][0-9A-Za-z_$]{21}$` is set on `id` in every class through `slot_usage`. For a parsed record it is the GlobalId of the IFC entity. For a derived record the derivation mints one in the same format. There is no separate `ifc_global_id` slot; `name` carries the readable label, and every reference slot holds an id. One exception since 2026-09-21: `Material`'s `id` is the material string as the parser derived it, because `IfcMaterial` has no GlobalId and within a model that string is its identity.

### Classes

| Class | Slots | Notes |
|---|---|---|
| `BuildingComponent` | `record_type`, `id`, `ifc_type`, `name`, `made_of`, `permanence`, `source`, `derived_from`, `part_of`, `target_location`, `supply_location`, `current_location`, `located_in`, `contained_in`, `length`, `width`, `height`, `weight` | One thing a task acts on. Every slot is `required: true`: the document shape is the same for every component. `ifc_type`, `made_of`, `derived_from`, `part_of`, `located_in`, `contained_in` hold `""` where they do not apply or are not yet resolved; `supply_location`, `current_location`, `length`, `width`, `height`, `weight` hold the unresolved sentinel, a real Position or Quantity object with `unit: ""`. One class rule: when `source` is `derived`, `derived_from` must additionally match the GlobalId pattern, not just be present. |
...
| `Material` | `record_type`, `id`, `category` | A material as IFC's `IfcMaterial`: one record per distinct material string the parser derives, shared by every component made of it. `id` is that string; `category` names the `MaterialCategory` a mapping step assigns after the parse. Every slot `required: true`; `category` is `""` until mapped. Added 2026-09-21, decision 17. |

All five classes carry an identifier and project to nodes. `derived_from`, `part_of`, `made_of`, `located_in`, `contained_in`, `connects`, and `category` are reference slots and project to edges, `category` to a `MaterialCategory` node. `Position` and `Quantity` have no identifier and flatten into the node that holds them.

Every class also carries `record_type`, a LinkML type designator (`designates_type: true`) that names the class itself: `"Storey"`, `"Space"`, `"BuildingComponent"`, `"Connector"`, `"Material"`. Declared once as a plain string slot and reused by all five the way `id` is; `Connector` inherits it from `BuildingComponent` without redeclaring it, and its own generated schema independently locks it to `"Connector"`, not `"BuildingComponent"` (LinkML resolves the designated value from the class actually being generated, not the class that declares the slot). A record therefore always says what it is, on its own terms, independent of which file holds it; a document validated against the wrong class fails on `record_type` even where the two classes' other slots would otherwise overlap.

### Enums

| Enum | Values | Notes |
|---|---|---|
| `ComponentPermanence` | `permanent`, `temporary` | Whether the component stays in the building. Reinforcement is permanent, formwork temporary. No default. Declared in common since 2026-09-22, because resource's `Stock` carries it too (decision 18); used here unchanged. |
...
| Slot | Range | Notes |
|---|---|---|
| `record_type` | string, `designates_type: true` | Which class the record is: `Storey`, `Space`, `BuildingComponent`, `Connector`, or `Material`. Required on all five; the value is fixed by the class, not authored. |
...
| `made_of` | `Material`, not inlined | The Material the component is made of, IFC's `IfcRelAssociatesMaterial` from the element's side. Required on every BuildingComponent; `""` when the parser derived no material string, a tier 3 work item. Named `made_of` because a slot `material` beside the class `Material` would write the same docs page. No pattern, because the id is a material string. |
| `category` | `MaterialCategory`, not inlined | The Material's category, IFC's `IfcMaterial.Category` as this project's vocabulary, the id of a record in common's `material_categories.yaml`: `masonry`, `timber`, `concrete`. Assigned by a mapping step after the parse. Required on every Material; `""` until mapped, a tier 3 work item. A plain string in JSON Schema; the join is checked by `tests/test_references.py` and by the generator at tier 2. |
| `permanence` | `ComponentPermanence` | Required, no default. Declared in common since 2026-09-22 (decision 18); `BuildingComponent` lists it and requires it through `slot_usage`. |
...
| `target_location` | `Position`, `inlined: true` | The centroid of the component's body in the IFC project frame, in the project length unit. Where the design puts it and what a placement task binds to, as the symbolic location `<id>_target` (`SPEC-process.md` decision 21). Required. |
| `supply_location` | `Position`, `inlined: true` | Where the component is delivered or staged; what a fetching task binds to, as `<id>_supply`. Required; the unresolved sentinel `{x_coord: 0.0, y_coord: 0.0, z_coord: 0.0, unit: ""}` when not yet known. |
| `current_location` | `Position`, `inlined: true` | Where the component is now, bindable as `<id>_current` when a plan starts from where execution left the component. Written by execution; the model's second runtime slot after `status`. Required; the same unresolved sentinel until execution writes a real one. |
...
Reused from common: `id`, `length`, `width`, `height`, `weight`, and since 2026-09-22 `permanence` as slots; `Position`, `Quantity`, `MaterialCategory`, `ComponentPermanence` as ranges. Every Position and Quantity slot states `inlined: true` so the document shape is visible in the YAML; a reference slot states `range` alone, no `inlined`, so a reader sees which slots are references. On `BuildingComponent`, `derived_from`, `part_of`, `located_in`, and `contained_in` additionally carry `pattern: "^$|^[0-3][0-9A-Za-z_$]{21}$"` in `slot_usage`, so their value is always a real GlobalId or the empty string, never other text.
...
- Every `BuildingComponent` key is written on every record, resolved or not: `ifc_type`, `made_of`, `derived_from`, `part_of`, `located_in`, and `contained_in` hold `""` rather than being left out when they do not apply or have not been resolved yet. Whether an empty one is expected (`part_of` on almost everything) or a gap worth a human's attention (`made_of`, `contained_in`, `located_in` on a model good enough to have them) is a tier 3 question, not a shape question.
- The parser writes one `Material` record per distinct material string it derives, with `category: ""`, and points every component at its record through `made_of`. A mapping step that runs after the parse and is not part of the derivation fills `category`, once per material, and leaves it `""` where it has no rule.
...
| IFC construct | Slot or class | Borrowed / Adapted | Note |
|---|---|---|---|
| IfcMaterial | `Material` | Adapted | One record per distinct material string, shared by the components made of it, as one IfcMaterial is associated to many elements. Description is not carried. |
| IfcMaterial.Name, or the set name the derivation rule reaches instead (IfcMaterialSelect) | `Material.id` | Adapted | The string the parser derives: occurrence over type, unwrap a usage to its set, IfcMaterial.Name, else the set's name, else member names joined with " / ". Loses layering, profiles, constituents. Rule in the research note, 3.3. IfcMaterial has no GlobalId, so the string is the record's identity. |
| IfcMaterial.Category | `category` | Adapted | IFC's optional category label, recommended values 'concrete', 'brick', 'wood' and so on; real files rarely fill it and the Duplex never does. Assigned by a mapping step into the project's `MaterialCategory` vocabulary, not read from the file. |
| IfcRelAssociatesMaterial | `made_of` | Adapted | The element's side of the association, one Material per component, IFC's MaxOneMaterialAssociation. |
...
| IfcProduct's "permanent use or temporary use", prose only | `permanence` | Adapted | IFC names the distinction in prose and has no attribute; the enum was the module's own until 2026-09-22, when it moved to common for resource's `Stock` (decision 18). |
...
| `Material.id`, `category` as keys, on every record | A material the parser met is a record whether or not the mapping step has reached it; `""` in `category` is the work item, and a missing key would hide it. |
...
- record_type: Material
  id: "Basic Wall:Exterior - Brick on Block"
  category: masonry
- record_type: Material
  id: Brick
  category: masonry
- record_type: Material
  id: "Metal - Steel - 345 MPa"
  category: ""
```
...
- record_type: BuildingComponent
  ...
  made_of: "Basic Wall:Exterior - Brick on Block"
...
Five classes, one `is_a`. `BuildingComponent` subclassed by kind only, never by source or permanence. `permanence` and `source` required with no default. `id` is the GlobalId, parsed or minted. `derived_from` is a required key on every record, `""` unless `source` is `derived`, when a rule requires it to hold a real id; the producing algorithm not recorded. `made_of` a required key naming a `Material`, `""` when the parser derived none, still a tier 3 query; `Material` shaped on `IfcMaterial`, one shared record per distinct string, its `id` the string and its `category` a required reference to common's `MaterialCategory`, `""` until the mapping step assigns one, the work item method selection reads (2026-09-21). `target_location` required and a centroid; `supply_location` and `current_location` also required keys, the unresolved sentinel (a real Position with `unit: ""`) until known. The exterior is a Space per storey; `connects` exactly two. `Storey` a class identified by its GlobalId. `name` and `long_name` are required keys everywhere they appear, `""` when the IFC file has none. `contained_in` ranges Storey and is reused for Space; `located_in` is the component-to-Space relation. Connectors are doors, voids, and stairs; a stair's clearances are optional. Instances are derived, not authored; the derivation is a later slice.
...
- **IFC names stay recognisable.** `ifc_type`, `name`, `long_name`, `contained_in`, `elevation`, `Material`, and `category` are IFC's words in the house style. The module's own words are `located_in`, `connects`, `kind`, `derived_from`, `part_of`, `made_of`, `source`, `record_type`, and the three location slots; `permanence` was one until it moved to common on 2026-09-22.
...
No new test files. Sixteen rows in `EXAMPLES`, fourteen on 2026-09-15 and two on 2026-09-21.

| Document | Class | Expected | Proves |
|---|---|---|---|
| `examples/product/building_components.yaml` | `BuildingComponent` | validates | IFC-sourced and derived components, a temporary one, a window, a part with `part_of`, the dimension slots, `located_in` and `contained_in`, `made_of` naming a Material on every component the model gives one and `""` on the rest, and the required-everywhere convention on every record, ifc-sourced or derived |
...
| `examples/product/materials.yaml` | `Material` | validates | One record per distinct material string in the component and connector files, `category` mapped on the masonry, concrete, and timber ones and `""` on the rest |
...
| `invalid/material_missing_category.yaml` | `Material` | fails on `category` | `category` is a required key on every Material |
...
What the validator does not check, and who does: that `connects`, `located_in`, `contained_in`, `derived_from`, `part_of`, and `made_of` name ids that exist in the sibling files is a tier 2 check for the projection component. Until then, success criterion 5 checks it by hand. The whole-graph checks this module needs are listed under Deferred to CHECKS.md below.
...
| Work list: every component with `made_of: ""` | all components | Fixed by the brief as a work list, not a shape error; `""` marks it, not a missing key |
| Work list: every Material with `category: ""` | all Materials | The mapping step has no rule for the string; one line per material, not per component, which is the list the catalogue author needs |
...
**Never.** Subclassing `BuildingComponent` by `source` or `permanence`. A slot named `material` beside the class `Material`. A pattern on `made_of`. A GlobalId or a second identity slot on `Material`. A `Material` without a `category` key. A default on `permanence` or `source`. An `id` that is not in GlobalId form, or a second identifier slot beside it. A `derived_by` or any record of the producing algorithm. A separate Opening node beside a door. Geometry, property sets, or a material taxonomy. `abstract`, `mixins`, or `union_of`. Graph vocabulary or annotations. Hand edits under `dist/` or `docs/model/`. Redefining a slot common declares. Invented example data.

## Success Criteria

1. `uv run pytest` passes with fourteen new rows collected and passing, sixteen since 2026-09-21, and no test file other than `test_examples.py` changed.
2. `dist/product.schema.json` declares draft 2020-12, passes the meta-schema check, and its `$defs` contain exactly `BuildingComponent`, `Connector`, `Space`, `Storey`, `Material` since 2026-09-21, `ComponentPermanence`, `RecordSource`, `ConnectorKind`, and common's `Position`, `Quantity`, `Interval`, `CapabilityType` (`ParameterKind` until it moved to process, 2026-09-18; `MaterialCategory` since 2026-09-21). `id` carries the GlobalId pattern in all four class definitions; `Connector.properties` holds every BuildingComponent property plus its four own; `connects` has `minItems: 2` and `maxItems: 2`; `BuildingComponent` and `Connector` each carry a bare `if`/`then` pair, the one `derived_from` rule, which `Connector` inherits and adds nothing to. All probed on 2026-09-15 and 2026-09-16.
3. `dist/README.md` lists `product.schema.json` with the module description.
4. `docs/model/product/index.md` lists the five classes with `Connector` indented under `BuildingComponent`, the two enums of its own since 2026-09-22, `RecordSource` and `ConnectorKind`, and this module's own slots, every entry with a description. The `BuildingComponent` page shows a Rules table; `Connector`'s does not, since `gen-doc` renders a rule only on the class that declares it, and the inherited one shows in the JSON Schema instead. The folder also holds unlinked pages for common's elements, as in resource.
5. The five example files come from one real model. `building_components.yaml` holds at least one IFC-sourced component whose `made_of` names a Material and one with `made_of: ""`, one window, one derived component whose `derived_from` names a component in the same file, one temporary component, and, when the model has an element assembly, one part whose `part_of` names its assembly in the same file or in `connectors.yaml`. `connectors.yaml` holds at least one door with clearances, one door whose `connects` names an exterior Space, and one stair. `spaces.yaml` holds one exterior Space per storey with `source: derived` and every id named in `connects` and `located_in`. `storeys.yaml` holds every id named in `contained_in`. `materials.yaml` holds every id named in `made_of` in the other two files, at least one with a real `category` and one with `category: ""`. Every `id` across the five files is distinct.
...
7. `schema/common.yaml` did not change during the module's four tasks, and the toolchain did not change. `git log -- schema/common.yaml scripts tests/test_build.py tests/test_lint.py tests/test_dist.py` shows no commit from this module. The 2026-09-21 amendment added `MaterialCategory` to common in its own commit before `Material` landed here, both under the process plan.
...
17. **`Material` as a class shaped on `IfcMaterial`, and `made_of` on the component, added 2026-09-21** during process's Phase 4, when method applicability moved to `Method.applies_to` as one `MaterialCategory` (`SPEC-process.md` decision 19). A catalogue cannot be written against strings like "Basic Wall:Exterior - Brick on Block", so a mapping step outside the schema turns each string into one of the project's categories, and methods are written against those. Where the category lives was decided from IFC: `IfcMaterial` carries `Name` and `Category`, so the category is an attribute of the material, not of the component, and `Material` is a class with `id`, the string, and `category`. Two shapes were weighed. An inlined object on every component, `material: {name, category}`, would have flattened to the same node properties as a bare `material_category` slot and made the mapping step write 143 copies of about twenty decisions. The identified, shared record is IFC's own shape, one `IfcMaterial` associated to many elements: the mapping step decides once per material, its work list is one line per material, and the component records keep the string they always carried, now as a reference. Its costs: `Material.id` is the one product id that is not a GlobalId, because `IfcMaterial` has none; a fifth example file; a third hop in the brief's opening query. The component's slot is `made_of`, not `material`, because a slot and a class of one name write the same generated docs page on a case-insensitive filesystem, and the repo renames the slot in that case. `category` ranges `MaterialCategory` in common, a class with its own document (`SPEC-common.md` decision 17; it was an enum for one day); it is a required key whose unmapped value is `""`, as decision 2 fixes for every reference slot, and the join is guarded by `tests/test_references.py` since nothing in JSON Schema checks a reference by id. `made_of` carries no pattern, because a material string has no fixed shape. `materials.yaml` is produced by the throwaway extractor from the distinct strings it meets, with a string-to-category table: brick-on-block and CMU walls to `masonry`, foundation walls and slabs on grade to `concrete`, wood floors and the wood-joist roof to `timber`, everything else `""`.
18. **`ComponentPermanence` and `permanence` leave for common, 2026-09-22.** Decided 2026-09-21 in Phase 1 of the `Stock` extension, `SPEC-stock.md` decisions 3 and 5. Resource's `Stock` is temporary when a unit comes back after a method's use, formwork, and permanent when it is consumed, rebar: product's own distinction, so the enum and the slot move to the module both product and resource import, as the four dimension slots did (`SPEC-common.md` decision 11), with their wording unchanged. Nothing here changes in behaviour: `BuildingComponent` lists `permanence` and requires it through `slot_usage`, the example documents are untouched, and `dist/product.schema.json` is the same in content, since imports merge. A stock is not joined to the component it becomes (`SPEC-stock.md` decision 8): product and resource still never reference each other.

19. **The three Position slots are the positions a `location` parameter binds, by name, 2026-09-23.** Decided in Phase 1 of the MVP dashboard, `SPEC-process.md` decision 21. Nothing in this module changes: `target_location`, `supply_location`, and `current_location` stay inlined Positions on the component, and a process binding names one of them as the component id followed by `_target`, `_supply`, or `_current`. A `Location` class was considered and declined, because it would hold the same coordinates a second time or turn these three slots into references, and the author wants a component's positions to be its own properties. The consequence for the derivation: `supply_location` is what a fetching task binds, so a model whose components carry the unresolved sentinel there needs a site setup step that writes one before planning; the parser never invents it.
