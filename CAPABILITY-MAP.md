...
Amended 2026-09-21, Phase 1 of the `Stock` extension (brief, Decisions Taken by Default; `pddl-for-process.md` Appendix C): `ResourceEntry` base and `Stock` added to `resource`, `ComponentPermanence` moves from `product` to `common`, `uses` added to `process`'s `Method`. Specified in `SPEC-stock.md`, the extension spec of `resource`, with the common and process changes as its prerequisites.
