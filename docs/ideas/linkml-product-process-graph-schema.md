...
The model is one schema split across four files joined by `imports`: `common.yaml` for shared types and kinds such as Position and Quantity, and for CapabilityType, since both the process side (`requires`) and the resource side (`offers`) point at it; `product.yaml` for BuildingComponent, Space, and Opening; `process.yaml` for PrimitiveTask, CompoundTask, Method, and the instantiated tasks; `resource.yaml` for RobotType and RobotUnit. Product and resource import only common. Process imports common, product, and resource: methods match component materials, primitives reference components, and the instantiated primitive carries `assigned_unit`, the one edge that records stage-two allocation. Nothing imports process. The assignment is stored once, on the task; "all tasks for this robot" is a query over the same edge, not a second slot. The split is for readability. Validator, generator, and documentation see a single model.
...
- **Resource side: the Construction Robot Schema** from "A Planning Schema of On-Site Construction Robot Operation." Borrow its physical, operational, safety, and activity attribute groups as the starting field set for `RobotType`. Adapt units to the model's quantity kind, split type from unit, and relocate attributes the process side already owns, such as capabilities, so nothing is declared twice.
...
The graph path this yields, and the picture the paper should open with: BuildingComponent, matched by material, `REFINED_BY` a Method, which `DECOMPOSES_INTO` PrimitiveTasks, each of which `REQUIRES` CapabilityTypes that a RobotType `OFFERS`. Product, process, and resource in one query.
...
| Requires capability | PrimitiveTask requires CapabilityType; RobotType offers it | Type-level task allocation | First |
...
| Resource availability | RobotUnit is free during the task's duration | Scheduling | Later |
| Capacity | A Quantity parameter of the task compared against a RobotType attribute, such as component weight against load capacity | Rejecting robot types no unit of which can do the task, before stage two | Later |
...
**Planning allocates to a type.** HDDL-level planning uses only the `requires` / `offers` pattern: this primitive needs lifting and alignment, and RobotType X offers both, so "one of the five robots of type X" can do it. The planner never sees individual robots. This is what keeps the planning problem tractable and what `RobotType` exists for.

**Orchestration allocates to a unit.** At execution time a separate step, using multi-robot task allocation algorithms rather than HDDL, refines the type-bound plan to specific robots: "of the five robots of type X, X2 is most suitable," judged on reach, load capacity, current position, battery, and availability. This is what `RobotUnit` exists for, and where the Construction Robot Schema's quantitative attributes earn their place. Type-level attributes such as maximum payload live on `RobotType`; unit-level state such as position and availability lives on `RobotUnit`, which points at its type.
...
Neither stage-two allocation nor the BT mapping is native to HDDL. Both are their own algorithms and their own specs. They are in this brief because they fix what the resource model has to carry and where the type/unit boundary falls.
...
The two questions the topology answers are queries, not model logic, and belong to stage two: "does this robot fit through that door" compares a `RobotType`'s Construction Robot Schema dimensions with an `Opening`'s clearance; "how to move from A to B" is a shortest path over Space adjacency.
...
- [ ] **The type/unit split in the resource model is clean.** Test: assign every Construction Robot Schema attribute to `RobotType` or `RobotUnit`. Any attribute that wants to be on both is a modelling problem to settle now.
...
- **Guessing in the generator.** Unprojectable constructs are rejected with a reason. A permissive generator produces a graph nobody can explain.
...
- **Unit-level detail in the planning model.** The planner sees types. Reach, load, position, and availability belong to `RobotUnit` and stage two.
...
- `RobotUnit` and the assigned-unit slot are in the first slice so stage two has a target.
...
- `offers` is a flat set of capability types. Type-level allocation is set containment. The known gap, a type that offers `lifting` but whose load capacity is below the component weight, is recorded as the Capacity relation kind, Later. Decided 2026-09-10.
...
- Stage-two allocation is one edge, `assigned_unit` on the instantiated primitive. No mirror slot on `RobotUnit`; the reverse direction is a query. Decided 2026-09-10.
