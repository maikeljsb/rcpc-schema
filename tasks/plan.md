    ...
    └── dist/process.schema.json, dist/README.md, docs/model/process/
            ...
            └── + Method, Subtask with its rule, compound_task, primitive_task, subtasks,      (Task 2)
                  ordering, arguments, applies_to
                    ...
                    └── dist/, docs/  rebuilt
                            │
                            └── common: + MaterialCategory, - MaterialName; product: + Material,      (Task 3, three commits)
                                  material -> made_of; process: applies_to leaves Parameter for Method
                                    ├── examples/product/materials.yaml new; components and connectors regenerated
                                    └── Task 3b: MaterialCategory enum -> class + material_categories.yaml; category "" until mapped;
                                        tests/test_references.py resolves requires, offers, category, applies_to, made_of
                                    ├── catalogue.yaml: m_prefab rewritten
...
- **The plan file lands whole in Task 4.** A plan is one thing a planner hands over; a network without its instances would validate but bind nothing. The nine records are written together and checked once against the tier 2 rows.
...
- **Task 3 is three commits in dependency order, common, product, process**, because each module's spec owns its change (`SPEC-common.md` decisions 15 and 16, `SPEC-product.md` decision 17, `SPEC-process.md` decision 19) and the build regenerates every importing module's `dist/` and docs with each. The product examples are the one exception to "every example file is written by hand": they were generated from the Duplex by the gitignored extractor under `examples/ifc_models/`, and `materials.yaml` is produced by giving that extractor a string-to-category table and regenerating, with `material` renamed `made_of` in the component and connector files, so the files stay reproducible from the model. The table: `Basic Wall:Exterior - Brick on Block`, the CMU party-wall string, and `Masonry - Brick` to `masonry`; `Basic Wall:Foundation - Concrete (417mm)`, `(435mm)`, `Floor:127mm Slab on Grade`, and `Floor:150mm Exterior Slab on Grade` to `concrete`; `Floor:Finish Floor - Wood`, `Floor:Residential - Wood Joist with Subflooring`, and `Basic Roof:Live Roof over Wood Joist Flat Roof` to `timber`; every other string, the steel beams, gypsum ceilings, stud partitions, and tile floors, to `""`; the empty string is no Material at all, and the component's `made_of` is `""`. Steel is the obvious fourth category; it is not added because no method in the catalogue is written for it, and the vocabulary grows with methods.
- **Shared vocabularies are classes with documents, and the join is a test.** Decided 2026-09-21 in the Task 3 review, `SPEC-common.md` decision 17, reversing that day's enum: a company adopting the schema adds a capability or a category in `examples/common/*.yaml`, never in the schema, and an enum is kept for values code branches on. A reference by id is a plain string in JSON Schema, so Task 3b adds `tests/test_references.py`, one row per document and slot, resolving `requires`, `offers`, `category`, `applies_to`, and `made_of` against their documents on every run; Task 4 adds rows for `plan.yaml`'s `compound_task`, `primitive_task`, `decomposed_by`, and `bound_to`. Without those rows the design is not type safe, and the spec says so.
- **The case-collision check is done on names, not on the folder listing.** On this Windows checkout gen-doc writes two pages that differ only by case into one file, so listing `docs/model/process/` cannot see the collision that Linux CI will. Tasks 3, 4, and 5 instead lowercase every class, slot, enum, type, and schema name that `SchemaView` reports for the module with imports and assert no two coincide. Probed 2026-09-18: the spec's first shape had a `position` slot, whose page overwrote common's `Position.md` here; the current shape has no such pair, checked on the probe schema. `Material` beside a slot `material` would be such a pair, which is why the component's slot is `made_of`; `MaterialCategory` beside `category` is not one.
- **Success criterion 5 is a one-off script in the scratchpad**, not committed, as product's criterion 5 was. It reads the catalogue, the plan, and the sibling example files and prints one line per tier 2 row of the spec; the todo records its output once. Task 3 runs the two rows it can already check, that `applies_to` names a material category and that the bound wall carries it.
...
|---|---|---|
| A process name collides case-insensitively with an imported one in the generated docs; Windows hides it, Linux CI fails the drift test | High if it happens: CI red on the commit | The name scan in Tasks 3, 4, and 5; the current names were scanned on 2026-09-18 and are clear, and `Material` against `made_of`, and `MaterialCategory` against `category`, on 2026-09-21 |
...
| Thirteen rows, two of them split into three validator runs each, push the suite well past its current 80 s | Low | Measure in Task 4; report, do not trim |
| Regenerating the product examples through the extractor changes more than the new key, because the extractor or ifcopenshell drifted since 2026-09-16 | Medium: a noisy product commit | Diff the regenerated files against the committed ones before staging; anything but the added key is investigated, not committed |
| A tier 2 row fails on the hand-written plan file, for example a subtask list shorter than its method or a binding that disagrees with the compound's | Low: the spec walked the tree | The criterion 5 script is written in Task 4, not Task 5, so a mismatch is fixed with the file it is in |

## Parallelisation

None. Five sequential tasks; Task 3's three commits are ordered by import.

## Open Questions

None. The Phase 1 review of 2026-09-21 reopened `applies_to` and closed it as decision 19, adding Task 3. The plan review of 2026-09-18 settled two: the spec's `position` slot collided with common's `Position` class in the generated docs, resolved by decision 6, which dropped the slot with the shape it belonged to; and the catalogue tasks share a base `Task` as the instances share `TaskInstance`, decision 18, with `primitive_task`, `compound_task`, and the `Subtask` rule unchanged.
