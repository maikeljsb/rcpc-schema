...
**Acceptance criteria:**
- [ ] `uv run linkml-lint` reports no problems for `schema/common.yaml`, `schema/product.yaml`, `schema/process.yaml`; `materials.yaml` validates against `Material`; `material_missing_category.yaml` fails naming `category`
- [ ] Every record in `examples/product/building_components.yaml` and `connectors.yaml` carries `made_of` and no `material`; every non-empty value is an id in `materials.yaml`, which holds one record per distinct string; there the brick-on-block string, the CMU party-wall string, and `Masonry - Brick` are `masonry`, the two foundation-wall and two slab-on-grade strings `concrete`, the two wood-floor strings and the roof `timber`, everything else `unassigned`; nothing else in the five files changed
- [ ] `dist/process.schema.json`: `Parameter.properties` holds `parameter_name` and `parameter_kind` and nothing else, the key slot stays a property; `Method.properties` holds `applies_to` and `Method.required` does not list it; `$defs` has 27 entries with `MaterialCategory` and `Material` among them, 32 only after Task 4; `dist/common.schema.json` and `dist/resource.schema.json` each gain the `MaterialCategory` enum with four values and nothing else; `dist/product.schema.json` gains `MaterialCategory` and `Material`; `Material.properties.category` and `Method.properties.applies_to` are each a `$ref` to `MaterialCategory`; `BuildingComponent.required` and `Connector.required` list `made_of` and not `material`; `Material.required` lists `category`, `id`, `record_type`

**Verification:**
- [ ] Tests pass: `uv run pytest` with two new rows collected, 59 in all, after each of the three commits
- [ ] Build succeeds: `uv run python scripts/build.py` run twice after each commit; `git status --porcelain` shows only the intended files
...
## Checkpoint: Phase 2
- [ ] 59 tests pass; the three commits are in the log in the order common, product, process
