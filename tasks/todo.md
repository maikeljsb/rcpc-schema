...
- [ ] Every record in `examples/product/building_components.yaml` and `connectors.yaml` carries `made_of` and no `material`; every non-empty value is an id in `materials.yaml`, which holds one record per distinct string; there the brick-on-block string, the two CMU strings, and `Masonry - Brick` are `masonry`, the two foundation-wall and two slab-on-grade strings `concrete`, the two wood-floor strings and the roof `timber`, everything else `unassigned`; nothing else in the five files changed
- [ ] `dist/process.schema.json`: `Parameter` has `parameter_kind` as its only property; `Method.properties` holds `applies_to` as a plain string and `Method.required` does not list it; `$defs` has 32 entries with `MaterialCategory` and `Material` among them; `dist/common.schema.json` and `dist/resource.schema.json` each gain the `MaterialCategory` enum with four values and nothing else; `dist/product.schema.json` gains `MaterialCategory` and `Material`; `Material.properties.category` and `Method.properties.applies_to` are each a `$ref` to `MaterialCategory`; `BuildingComponent.required` and `Connector.required` list `made_of` and not `material`; `Material.required` lists `category`, `id`, `record_type`
