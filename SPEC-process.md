...
examples/
  process/
    ...
    plan.yaml                                               one network of two walls, the second after the first; the two
                                                            network tasks, one child compound, and the five leaves
    invalid/
      ...
      primitive_task_instance_without_parent.yaml           parent: ""
dist/
...
The **catalogue** (`PrimitiveTask`, `CompoundTask`, `Method`) is the HDDL domain, one file, hand-authored: its slots are required where a missing value would leave the task unusable, and optional otherwise. The **plan side** (`TaskNetwork`, `CompoundTaskInstance`, `PrimitiveTaskInstance`) is the HDDL problem's initial task network and the planner's solution, one file per plan, and follows the required-everywhere rule product adopted for pipeline output: every slot is a required key, and an unresolved or inapplicable value is `""` on a string or reference slot, `[]` on a list, `0` on the one integer that has a network-task case. A reference holding `""` renders as no edge.

### Classes

| Class | Slots | Notes |
|---|---|---|
| `PrimitiveTask` | `record_type`, `id`, `description`, `parameters`, `requires`, `duration` | HDDL's action. `id` is the action name, matching `^[A-Za-z][A-Za-z0-9_]*$`, and is what a behaviour tree registers. `parameters` is a keyed map of `Parameter`; every primitive declares a parameter of kind `robot`. `requires` is the one condition the model states. Required: everything. |
| `CompoundTask` | `record_type`, `id`, `description`, `parameters` | HDDL's abstract task: a name and typed parameters, no body, no state change. Same `id` pattern. Required: everything; `parameters` may be `{}`. |
...
| `TaskNetwork` | `record_type`, `id`, `tasks`, `ordering` | HDDL's `:htn` block: what the plan is asked to accomplish. `tasks` is the ordered list of the parentless `CompoundTaskInstance`s, at least one, each position its label; `ordering` is the same list of position pairs a method carries, `[]` when the tasks may interleave. Every slot required. |
| `CompoundTaskInstance` | `record_type`, `id`, `compound_task`, `decomposed_by`, `parent`, `position`, `bindings` | One planned occurrence of a compound task: the IPC decomposition line. A task of the network has `parent: ""` and `position: 0`; a child names the compound instance it was decomposed from and which subtask of that compound's method it realises, from 1. `decomposed_by` is `""` until the planner has chosen a method, then that method. `bindings` binds exactly its task's parameters, decomposed or not; the method's own variables are bound on the children. Every slot required. |
| `PrimitiveTaskInstance` | `record_type`, `id`, `primitive_task`, `parent`, `position`, `bindings` | One planned occurrence of a primitive task: the IPC plan line `<id> <action> <args>`. `parent` is never empty, a primitive is always a leaf under a compound; `position` is which subtask of the parent's method it realises, from 1. `bindings` binds every parameter of its primitive task. Every slot required. |
...
Six classes carry an identifier and project to nodes: the three catalogue classes, the network, and the two instance classes. `Parameter`, `Subtask`, and `Binding` have none and flatten into the record that holds them. `compound_task`, `primitive_task`, `requires`, `tasks`, `decomposed_by`, and `parent` are reference slots and project to edges. `bound_to` projects to an edge by the second catalogue-keyed rule (prerequisite 4). `Quantity` flattens as in every module.

The six identified classes carry `record_type`, product's type designator, reused as `id` is; LinkML locks its value to the class being validated, probed for all six. Here it is load-bearing: a catalogue file and a plan file each hold records of three classes, and `record_type` is what routes a record to its class for validation and, later, in the generator. `Parameter`, `Subtask`, and `Binding` do not carry it, for the same reason `Position` does not: they are never a record on their own and never validated against directly.
...
| `subtasks` | `Subtask`, multivalued, `inlined: true`, `inlined_as_list: true`, `required: true`, `minimum_cardinality: 1` | The method's subtasks in list order. `[]` fails naming `subtasks`. Probed. |
...
| `parent` | `CompoundTaskInstance` | The compound instance this one was decomposed from. Required on both instance classes; on `PrimitiveTaskInstance` additionally `pattern: "^.+$"`, so `""` fails naming `parent`; on `CompoundTaskInstance` `""` marks a task of the network. |
| `position` | integer | Which subtask of the parent's method the instance realises, from 1. Required on both; `minimum_value: 1` on `PrimitiveTaskInstance`, `minimum_value: 0` on `CompoundTaskInstance`, where `0` marks a task of the network. |
| `bindings` | `Binding`, multivalued, `inlined: true` | The instance's parameters bound to object ids, keyed by parameter name: `{r: mason_m1_1, to: 0BTBFw6f90Nfh9rP1dlXrr}`. Required on both instance classes. Dict form with the one-line value; an object or a list in its place is rejected, probed. |
...
| `(:htn :tasks (and ...) :ordering (...))` (1.9) | `TaskNetwork` with `tasks`, `ordering` | Borrowed | The same `<tasknetwork-def>` a method body uses: a labelled list and pairs over the labels. Its tasks are the `CompoundTaskInstance`s with `parent: ""`. |
...
| decomposition line `<id> <task> <args> -> <method> <children>` (1.10) | `CompoundTaskInstance` with `compound_task`, `bindings`, `decomposed_by`; children by `parent` and `position` | Adapted | The child names its parent and its position instead of the parent listing children, because the children are of two classes and one reference list cannot range both. As on the IPC line, the compound carries its task's arguments only; the method's own variables appear on the children. |
| ordering among a compound's children (Def. 5) | none written | Borrowed | Inherited from the method through `position`; nothing to drift. |
...
| Slot | Why |
|---|---|
| `record_type` on the six identified classes | A record says what it is; a document validated against the wrong class fails on `record_type` specifically. |
| `id` on the six identified classes, the catalogue three matching the name pattern | Nodes need identifiers; catalogue names travel unchanged into HDDL and a behaviour tree. |
...
| every slot of both instance classes | Planner output has the same shape on every record; `""` and `0` mark the network-task and undecomposed cases so a check reads a value, never tests for a key. |
| `PrimitiveTaskInstance.parent` non-empty, `position` at least 1 | A primitive is always a leaf of some compound; the network holds compounds only. |
...
`plan.yaml` holds the network, its compounds, and its leaves in one list. The network, the three compounds, and the first of five leaves:
...
- **HDDL's words where HDDL has one.** `PrimitiveTask`, `CompoundTask`, `Method`, `Subtask`, `TaskNetwork`, `parameters`, `subtasks`, `tasks`, `ordering`, `duration` are HDDL's. `compound_task` stands for `:task` because a slot named `task` would carry two ranges across three classes; `arguments` stands for the parameter sequence because `parameters` already names the declaration map; `requires`, `applies_to`, `bindings`, `bound_to`, `decomposed_by`, `parent`, `position` are the module's own plain words.
...
- **One ordering mechanism.** Wherever tasks are ordered, on a method or on a network, it is a list with positions from 1 and `ordering` as pairs of positions. Nothing else orders anything.
...
| `examples/process/plan.yaml` | by `record_type` | validates | One network with two tasks and one ordering pair; a decomposed network task, a child compound, an undecomposed network task; five leaves with one-line bindings of every kind; all three classes in one file |
...
| `invalid/primitive_task_instance_without_parent.yaml` | `PrimitiveTaskInstance` | fails on `parent` | A primitive is never a task of the network |
...
| Check | Reads |
|---|---|
| Every `compound_task`, `primitive_task`, `decomposed_by`, `parent`, `tasks`, and `requires` value names a record of its range; `""` excepted where allowed | the catalogue and plan files |
...
| Every `CompoundTaskInstance` with `parent: ""` is listed in exactly one network's `tasks`; no child is listed in any | `TaskNetwork.tasks`, `parent` |
...
| A decomposed compound has exactly one child per subtask position of its method, each child's task the one the subtask names; an undecomposed compound has no children | `parent`, `position`, `Method.subtasks` |
| Through the parent's method, each subtask argument names a method variable or an object id. Where it names a variable that is also a parameter of the parent's task, the child's binding equals the parent's; where two children's arguments name the same method variable, their bindings are equal; where it names an object id, the child's binding is that id. The `robot` variable is exempt from the equality within one entry, so stage two may rewrite one leaf | both instance classes, `Subtask.arguments`, `Method.parameters` |
...
2. **A list entry with a reference slot** yields one edge from the owner to the referenced node carrying the entry's position and its other primitive slots: `Method -[position, arguments]-> PrimitiveTask | CompoundTask`, two parallel edges when the same task occurs twice. This is the existing reference-slot rule applied inside an ordered list.
...
**Ask first.** Adding a class or slot beyond the tables above. A fourth `ParameterKind` value. A slot for effects, preconditions, or component state. A planned or actual duration on an instance. A `Plan` or `Run` class beside `TaskNetwork`. A second class rule on any class. A pattern on `parameter_name`, `bound_to`, or instance ids. Making `applies_to` required anywhere. A primitive as a task of the network. Moving anything into common. Touching `schema/product.yaml` or `schema/resource.yaml` beyond prerequisite 2's example file. Adding a test file.

**Never.** A formula language or a string that is one. Method preconditions or effects. `Subtask` or `Binding` as identified classes. `assigned_unit` or any mirror slot on `RobotUnit`. Ordering between a compound's children in a ground document. A third rule keyed on the model's data in the projection. `abstract`, `mixins`, or `union_of`. `any_of` ranges. Graph vocabulary or annotations. Hand edits under `dist/` or `docs/model/`. Redefining a slot common or product declares. Real robot products in the examples.
...
2. `dist/process.schema.json` declares draft 2020-12, passes the meta-schema check, and its `$defs` contain exactly this module's `PrimitiveTask`, `CompoundTask`, `Method`, `Parameter`, `Parameter__identifier_optional`, `Subtask`, `TaskNetwork`, `PrimitiveTaskInstance`, `CompoundTaskInstance`, `Binding`, `Binding__identifier_optional`, `ParameterKind`, and every class and enum of common, product, and resource (imports merge), 30 entries. `record_type` is an `enum` of one value in each of the six identified classes; `requires`, `subtasks`, and `tasks` carry `minItems: 1`; `parameters` and `bindings` are objects whose `additionalProperties` is an `anyOf` of the entry object and its one-line value; `Subtask` carries a bare `if`/`then`; the string `"null"` occurs exactly twice, inside `ordering.items.type` on `Method` and on `TaskNetwork`. All probed 2026-09-18.
3. `dist/README.md` lists `process.schema.json` with the module description.
4. `docs/model/process/index.md` lists exactly nine classes, seventeen slots, and `ParameterKind`, every entry with a description; `Subtask.md` shows a Rules table. No two file names under `docs/model/process/` match case-insensitively (checked on the probe: 133 pages, none). The folder also holds unlinked pages for the imported modules' elements, as in product and resource.
5. Checked once by a one-off script and recorded: every `requires` id is in `examples/common/capability_types.yaml`; every `compound_task`, `primitive_task`, and `decomposed_by` value in `plan.yaml` resolves in `catalogue.yaml`, and every `parent` and `tasks` value within `plan.yaml`; every `bound_to` under a `component` or `location` parameter resolves in `examples/product/` and every one under a `robot` parameter is `mason_m1_1` or `mason_m1_2`; the example tree passes every tier 2 row above by hand, including the child-binding agreement; `m_prefab`'s `applies_to` contains the bound wall's `material`; `mason_m1` offers everything `MoveTo`, `Attach`, `Detach`, and `Place` require.
...
5. **Each instance binds exactly its own task's parameters: the compound its compound task's, the primitive its primitive task's.** Decided in review 2026-09-18 as the shape truest to HDDL, replacing the draft in which the compound bound all of its method's variables. The IPC solution format, the attested HDDL output, writes a decomposition as `<id> <task> <args> -> <method> <child ids>`: the compound carries its task's arguments only, and the method's own variables (`r`, `from`, `to` in `m_transport`) exist only in the children's arguments. So `bindings` keys are always the task's parameters, the same before and after decomposition, nothing is bound twice, and stage two's rewrite of a leaf's `robot` conflicts with nothing on the parent. Agreement between records is a tier 2 check through the method's `arguments`: a variable shared by two children binds equal on both, a variable that is the parent task's parameter binds equal to the parent's, the `robot` variable equal within its entry. The brief's two sentences saying the compound "binds that method's variables once" and that the method's `robot` variable "is bound once on the CompoundTaskInstance and its children inherit it" are amended to this; its "no copy is written" stays true and now covers bindings as well as ordering.
6. **The tree is written child-up: `parent` and `position` on every instance.** Accepted. The IPC line lists children on the compound, but the children are of two classes and one reference list cannot range both without an `any_of`. A child naming its parent and the subtask position it realises carries the same information with single-range slots, and the position is what ties a child to its subtask and so to its inherited order.
7. **The initial task network is a record, `TaskNetwork`, shaped like a method body.** Decided in review 2026-09-18, replacing the first draft's `after` slot on each parentless compound. HDDL's `:htn` block reuses the method body's grammar, a labelled task list and ordering pairs, and the IPC solution format opens with a `root <ids>` line; both point at one record listing the network's tasks. `TaskNetwork` carries `tasks`, the ordered ids of the parentless compounds, and `ordering`, the same pair list a method carries, so the module has one ordering mechanism. `tasks` lists its members rather than the members naming the network, because `parent` ranges `CompoundTaskInstance` and cannot also range `TaskNetwork`; the network's tasks keep `parent: ""` and `position: 0`, and tier 2 checks that exactly those are listed. A step number per task was considered and set aside because it cannot write every partial order HDDL can.
8. **`record_type` only on the six identified classes**, not on `Parameter`, `Subtask`, `Binding`. Accepted. The brief says "every process class". The three are inlined value-like objects never validated on their own, like `Position`; a designator inside every one-line entry would end the one-line form.
...
10. **A primitive instance is never a task of the network.** Accepted as the simplest shape. `parent` carries `pattern: "^.+$"` on `PrimitiveTaskInstance`, and `tasks` ranges `CompoundTaskInstance` only. HDDL's initial network may hold a primitive directly; here a standalone action needs a compound and a one-subtask method around it, as Transport wraps `noop` in `get-to` and `m-i-am-there`, though there for the sake of alternatives. The gain is one class of network task and a control node above every leaf in the behaviour tree. Allowing primitives at the top would need a second list on the network or an `any_of`, both declined.
11. **Patterns on two plan-side reference slots.** Accepted. `decomposed_by` names what HDDL Definition 5 (Höller et al. 2020, §2 p. 3 of the arXiv version) defines, the decomposition of a task by a method, which the IPC solution line writes as `-> <method>`; the slot name is ours, not a quotation. `parent` has no HDDL word, the IPC line listing children instead. `decomposed_by` takes the catalogue name pattern or empty; `parent` on a primitive is non-empty. The first draft also had GlobalId-or-empty and machine-id patterns on the three typed binding slots; with one `bound_to` slot for three id shapes there is nothing a single pattern can say, so the kind check is tier 2 alone.
