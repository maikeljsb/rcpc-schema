...
6. **`SPEC-toolchain.md` gains rule 8: one JSON Schema definition for a keyed class written only as dict values.** The dict form of `parameters` and `bindings` rests on a key slot, and LinkML's generator renders every keyed class twice: the class itself, with the key required, and a copy suffixed `__identifier_optional` with the key dropped, which is what the dict's values are checked against because the key carries the name. For `Parameter` and `Binding` the first definition is referenced by nothing, since neither is ever written on its own. The author required the dict form as input and a single definition in the output, both hard, on 2026-09-18; the list form was declined as not what a person writes. The build script therefore replaces the unreferenced definition with the copy under the class's own name. It touches no check: `linkml-validate` builds its own JSON Schema from the YAML, and the pruned definition was satisfiable by no correct record. Committed on its own, before this module's first task, with its test on `tests/fixtures/keyed.yaml`.
7. **`MaterialCategory` in common, `Material` and `made_of` in product.** Decided 2026-09-21 with decision 19. `SPEC-common.md` decision 15 adds the enum, `masonry`, `timber`, `concrete`, `unassigned`, each with a description; decision 16 retires the `MaterialName` type in the same commit. `SPEC-product.md` decision 17 adds the `Material` class shaped on `IfcMaterial`, one shared record per distinct material string with its `category`, `unassigned` until a mapping step outside the schema fills it, and renames the component's slot to `made_of`; the product examples gain `materials.yaml` through the throwaway extractor's string-to-category table. Each lands in its own commit, common then product, before this module's Task 3 moves `applies_to`.
...
| `applies_to` | `MaterialCategory` | The material category of the components a method decomposes its task for, one value such as `masonry`; a component matches when the Material it is `made_of` has that `category`. On `Method` only. Optional: absent when the method applies to any component; `unassigned` is never written here. Single-valued: the vocabulary is cut so that every method has one category, decision 19. |
...
- **One line for a declaration, one for a binding.** A `Parameter` is written `r: robot`; the object form `r: {parameter_kind: robot}` validates but says nothing more and is not written. A `Binding` is always the one-line `r: mason_m1_1`.
...
| Work list: every Material whose `category` is `unassigned` or one no method's `applies_to` names | The brief's example of a tier 3 rule |
