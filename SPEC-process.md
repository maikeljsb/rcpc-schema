...
Six: five decided in Phase 1 on 2026-09-17 or in this spec's review on 2026-09-18, all landing with this module's first task and amended in their own document in the same review; the sixth decided during Task 1 on 2026-09-18.
...
7. `schema/common.yaml` changed only by the removal of `ParameterKind`; `examples/resource/robot_units.yaml` changed only by one word in `mason_m1`'s `offers`; `scripts/build.py` and `tests/test_build.py` changed only by rule 8 (prerequisite 6), in its own commit; `schema/product.yaml`, `schema/resource.yaml`, `tests/test_lint.py`, and `tests/test_dist.py` did not change. `git log -- schema/product.yaml schema/resource.yaml tests/test_lint.py tests/test_dist.py` shows no commit from this module, and `git log -- scripts tests/test_build.py` shows the one prerequisite commit.
