...
examples/
  process/
    catalogue.yaml                                          MoveTo, Attach, Detach, Place, Formwork, Reinforce, Concrete;
                                                            ConstructComponent, Transport; m_transport, m_prefab, m_insitu:
                                                            every catalogue record, in one file (the in-situ three 2026-09-22)
    plan.yaml                                               one network of two walls, the second after the first, and two
                                                            slabs unordered (2026-09-22); the four network tasks, one
                                                            Transport compound, and the five leaves
    invalid/
...
| Enum | Values | Notes |
|---|---|---|
| `ParameterKind` | `component`, `location`, `robot` | The kinds of object a parameter ranges over: a `BuildingComponent` or its subclass; one of a component's three positions, named after the component and the slot as `<id>_target`, `<id>_supply`, or `<id>_current` (decision 21, 2026-09-23; until then a `Space`); a machine of a `RobotUnit` entry. Moved here from common with these values, replacing `position`, `component_reference`, `quantity`. No default. |
...
| `uses` | `Stock`, multivalued | The stocks a method holds one unit of each for its whole span, by id: `[formwork_panels, rebar]`. On `Method` only. Optional: absent when the method uses no stock. A plain string in JSON Schema, a reference by id like `requires`; resolved against `examples/resource/stocks.yaml` by `tests/test_references.py`. What the export does with it, a take before the first subtask and, for a temporary stock, a release after the last, is `SPEC-stock.md`'s. Added 2026-09-22, decision 20. |
...
Reused: `id`, `description` from common; `record_type` from product; `Quantity`, `CapabilityType`, `MaterialCategory`, and since 2026-09-22 resource's `Stock` as ranges. Not used, because product owns them with product's meaning and they are global: `name`, `kind`, `source`, `status`.
...
|---|---|---|---|
| `(:types ...)` (1.2) | `ParameterKind` | Adapted | Three object kinds: `component` and `robot` are identified classes in product and resource, `location` is a component's position, an object the export mints from the component id and the slot name as it mints machines from `count`. No hierarchy, no `either`. Numbers are not types (PDDL 2.1 §3), so `position` and `quantity` left the enum; a named position is an object, not a number (decision 21). |
...
| a typed object per stock unit with a `free` predicate, taken by one action and returned by another (Appendix C.2, C.3) | `uses` | Adapted | The method names the stocks; the export mints the variable, the take before the first subtask, and the release after the last for a temporary stock. No position within the method, no amount. Added 2026-09-22, decision 20. |
...
`catalogue.yaml` holds all twelve records in one list, in any order; `record_type` says what each is. Two primitives, one compound task, and the three methods:
...
`Attach(r, c, at)` and `Detach(r, c, at)` both require `[grip]`; `ConstructComponent` has `parameters: {c: component}` like `Transport`. `masonry` is the `category` of the brick-on-block Material the product examples' walls are `made_of`; `m_transport` has no `applies_to` and applies to any component. `m_prefab` has a compound subtask so the tree has three levels; `m_transport`'s two `MoveTo` entries are two positions and two edges. `Formwork`, `Reinforce`, and `Concrete` are primitives with `parameters: {r: robot, c: component, at: location}`, requiring `shutter`, `tie`, and `pour` respectively, durations 14400, 10800, and 7200 s; `m_insitu` is the second method for `ConstructComponent`, so `applies_to` selects between two method families for one task, and its `uses` names the two stocks in `examples/resource/stocks.yaml` (2026-09-22).

### One plan, as one document

`plan.yaml` holds the network, its compounds, and its leaves in one list, read top down as a method is read. The network, three of its five compounds, and the first of five leaves:

```yaml
- record_type: TaskNetwork
  id: level_1
  tasks: [construct_138157, construct_138062, construct_141232, construct_143106]
  ordering: [[1, 2]]
...
- record_type: PrimitiveTaskInstance
  ...
  bindings: {r: mason_m1_1, to: 2O2Fr$t4X7Zf8NOew3FNqI_supply}
```

The network asks for two walls, the second after the first, and, since 2026-09-22, for the two Level 1 slabs `2O2Fr$t4X7Zf8NOew3FK4F` and `2O2Fr$t4X7Zf8NOew3FKcz` as tasks 3 and 4, with no ordering pair among them or to the walls: whether they run side by side is for the planner to decide from the stock `m_insitu` uses. Each slab is an undecomposed `CompoundTaskInstance` shaped like `construct_138062`, bound to its slab. The first wall is decomposed by `m_prefab`, whose two lines are Transport then Place, so its `subtasks` has two entries: the Transport compound and the Place leaf. The Transport compound is decomposed by `m_transport`, four lines, so it lists four leaves; the first and third are both `MoveTo`, told apart by their place in the list, and ordered by the method's `ordering` applied to that list. The second wall and the two slabs are still undecomposed tasks, as a planner's input looks before it runs. One plan is one file: what the planner hands over and what stage two rewrites the robot bindings in. The component is the Level 1 wall `Basic Wall:Exterior - Brick on Block:138157` from `examples/product/`; the method's `from` and `to` are its `supply_location` and `target_location`, bound as `<id>_supply` and `<id>_target` (decision 21). `mason_m1_1` is the first machine of the `mason_m1` entry; it appears only on the leaves, because a compound binds its task's one parameter `c` and the method's variables `r`, `from`, `to` are bound where they are used. Instance ids are the listing compound's id with the entry's number appended, a convention of the example, not of the schema.

### Fixed by the brief

Three catalogue concepts under HDDL's names and no Task-as-plan-unit. Parameter kinds `component`, `location`, `robot`; `ParameterKind` in process; `location` ranges over a component's three positions, named `<id>_target`, `<id>_supply`, `<id>_current` (2026-09-23, decision 21). `applies_to` on the method, one `MaterialCategory`, equality (2026-09-21). `requires` as the one precondition; no effects, no method preconditions, no formula language; stock availability in as `uses` on `Method`, a list of `Stock` ids held for the method's span (2026-09-22). `duration` on `PrimitiveTask`. Subtasks as list entries, not nodes; ordering as position pairs on the method, whole tasks only. Ground classes `PrimitiveTaskInstance` and `CompoundTaskInstance`; ground ordering only among the network's tasks; a compound's subtasks inherit order from the method. The `robot` binding on the primitive instance is the allocation edge, bound by the planner, rewritten by stage two within the entry; no `assigned_unit`. Amended 2026-09-18: the compound binds its task's parameters, not the method's variables; see decision 5. `record_type` on process classes. The example instance proves the path component to method to primitive to capability to robot. MA-PDDL not adopted.
...
- **HDDL's words where HDDL has one.** `Task`, `PrimitiveTask`, `CompoundTask`, `Method`, `Subtask`, `TaskNetwork`, `TaskInstance`, `parameters`, `subtasks`, `tasks`, `ordering`, `duration` are HDDL's; `subtasks` is used on the method and on the compound instance because HDDL uses one word for the lifted and the ground network. `compound_task` stands for `:task` because a slot named `task` would carry two ranges across three classes; `arguments` stands for the parameter sequence because `parameters` already names the declaration map; `requires`, `applies_to`, `uses`, `bindings`, `bound_to`, `decomposed_by`, and the keyed-map names `parameter_name` and `parameter_kind` are the module's own plain words.
...
|---|---|---|---|
| `examples/process/catalogue.yaml` | by `record_type` | validates | Seven primitives, one `requires` of two, every primitive with a `robot` parameter; two compound tasks; a method with a total order of four and a repeated primitive and no `applies_to`, one with a compound subtask and `applies_to: masonry`, and since 2026-09-22 one with `applies_to: concrete` and `uses` of two; all three classes in one file |
| `examples/process/plan.yaml` | by `record_type` | validates | One network with four tasks and one ordering pair; a decomposed network task listing two subtasks, a decomposed compound listing four, three undecomposed network tasks listing none; five leaves with one-line bindings of every kind; all three classes in one file, `record_type` locking through `is_a` |
...
| Check | Reads |
|---|---|
| Every `compound_task`, `primitive_task`, `decomposed_by`, `tasks`, `subtasks` (on an instance), `requires`, and `uses` value names a record of its range; `""` excepted where allowed | the catalogue and plan files, `stocks.yaml` |
...
| Each `bound_to` names a record of the class the parameter's declared kind says: a `BuildingComponent` for `component`, a position id `<component>_<slot>` with `slot` one of `target`, `supply`, `current` and the component existing for `location`, a machine id `<entry>_<n>` with `1 <= n <= count` of an existing entry for `robot` | `ParameterBinding`, the declaration on the bound task, product and resource ids |
...
3. **A binding is an edge whose target class is the parameter's declared kind.** For each `bindings` entry the component looks up the parameter on the instance's `primitive_task` or `compound_task`, reads `parameter_kind`, and writes one edge carrying the key to the `BuildingComponent` or `RobotUnit` machine node the id names. The machine node is the one the `count` rule created; no document holds its id. This is the second rule keyed on the model's own data (prerequisite 4); a missing declaration is a tier 2 error before any write, so the rule never guesses.
4. **A `location` binding is an edge to the component whose position it names, carrying the slot.** `at: 2O2Fr$t4X7Zf8NOew3FNqI_target` splits at its last underscore into a component id and a slot suffix; the edge goes from the instance to that `BuildingComponent` node and carries the key `at` and the slot `target_location`. The coordinates stay on the component, where the position rule flattened them, one hop away; nothing is copied. No position node exists, as no machine record exists: the export mints the object `<id>_target` from the component, the plan writes it back, and the projection resolves it. This is the third rule keyed on the model's own data (decision 21, 2026-09-23); the brief records the reopening.
...
7. `schema/common.yaml` changed only by the removal of `ParameterKind` and, in prerequisite 7, the addition of `MaterialCategory` and the removal of `MaterialName`; `examples/resource/robot_units.yaml` changed only by one word in `mason_m1`'s `offers`; `scripts/build.py` and `tests/test_build.py` changed only by rule 8 (prerequisite 6), in its own commit; `schema/product.yaml` and its examples changed only by `Material` and `made_of` (prerequisite 7), in its own commit; `schema/resource.yaml`, `tests/test_lint.py`, and `tests/test_dist.py` did not change. `git log -- schema/resource.yaml tests/test_lint.py tests/test_dist.py` shows no commit from this module; `git log -- schema/product.yaml` shows the two prerequisite 7 commits, `Material` and then the `""` category of Task 3b; `git log -- scripts tests/test_build.py` shows the one prerequisite 6 commit; `git log -- schema/common.yaml` shows the `ParameterKind` removal, the one-day enum, the `MaterialName` removal, and the class of Task 3b.
...
20. **`uses` on `Method`: the stocks a method holds one unit of each for its whole span, optional, multivalued, range `Stock`.** Decided 2026-09-21 in Phase 1 of the `Stock` extension, `SPEC-stock.md` decisions 1, 2, and 7, from the four Aries runs in `docs/research/pddl-for-process.md` Appendix C; recorded 2026-09-22. The question was whether two in-situ walls could run their formwork side by side when enough exists and one after the other when it does not, without an imposed order. The model as it stood could not tell one formwork set from two: `requires` is a type-level check on the robot, and nothing a method names is consumed or held. A stock object with a `free` fact, taken by the first subtask and returned by the last, made the planner sequence on its own (C.3), so the method gets a list of the stocks it holds. Why on the method and not a parameter: units are interchangeable, there is no optimal panel as there is an optimal robot, so nothing is bound and the export mints the variable, the timed conditions, and the effects, a `ParameterKind` value and a binding both declined (`SPEC-stock.md` decision 1). Why a plain list and not positions: the first draft placed a take and a release at subtask positions; for `m_insitu` the positions equal the span, and elsewhere they only save idle stock time, never a wrong plan, so the entry is an id and positions are additive later as an object form (`SPEC-stock.md` decision 2). Optional, because the catalogue side is hand-authored and most methods use no stock, as `applies_to` is absent when a method applies to any (decision 9). The range is resource's `Stock`, a class this module already imports the module of; a reference by id renders as a plain string, so `tests/test_references.py` gains the row `catalogue.yaml` `uses` to `stocks.yaml`, the rule of `SPEC-common.md` decision 17. The in-situ method joins the masonry catalogue rather than a second file, so `applies_to` selects between two method families for one compound task, and the network gains the two slabs as an unordered pair beside its ordered one (`SPEC-stock.md` decision 7).

21. **`location` ranges over a component's three positions, by name: `<id>_target`, `<id>_supply`, `<id>_current`.** Decided 2026-09-23 for the MVP dashboard, reversing `docs/research/pddl-for-process.md` Part 4 question 4, where it ranged over `Space`. A task has to say where a component is fetched and where it goes; `supply_location` and `target_location` already hold both, and a Space can name neither, since `located_in` is empty on every wall and slab. The 2026-09-17 argument was PDDL 2.1's rule that numbers are not parameter values; a named location object is not a number, so the rule does not exclude it. No class or slot changes: the coordinates stay inline, the export mints the names as objects as it mints machines from `count`, tier 2 splits a name at its last underscore, and the projection writes the binding as an edge to the component carrying the slot, the third data-keyed rule (What the projection must add, item 4). There is no depot record: a component starts at its supply position, or at its current one once execution has written it. `Space` is no longer a binding target.
